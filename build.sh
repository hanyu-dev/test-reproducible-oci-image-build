...
IMAGE_SING_BOX_VERSION="${IMAGE_SING_BOX_VERSION:-}"
...
while [[ $# -gt 0 ]]; do
	...
	--image-build-target-goarchs)
		...
		shift 2
		;;
	--image-sing-box-version)
		IMAGE_SING_BOX_VERSION="$2"
		shift 2
		;;
...
if [ -z "$IMAGE_NAME" ] || [ -z "$IMAGE_REGISTRY" ] || [ -z "$IMAGE_BUILD_TARGET_GOARCHS" ] || [ -z "$IMAGE_SING_BOX_VERSION" ] || [ -z "$IMAGE_BUILD_REVISION" ]; then
...
IMAGE_SING_BOX_VERSION_MAJOR=$(echo "${IMAGE_SING_BOX_VERSION%%-*}" | awk -F. '{print $1}')
IMAGE_SING_BOX_VERSION_MINOR=$(echo "${IMAGE_SING_BOX_VERSION%%-*}" | awk -F. '{print $2}')
IMAGE_SING_BOX_VERSION_PATCH=$(echo "${IMAGE_SING_BOX_VERSION%%-*}" | awk -F. '{print $3}')

IMAGE_SING_BOX_VERSION_MAJOR=${IMAGE_SING_BOX_VERSION_MAJOR:-0}
IMAGE_SING_BOX_VERSION_MINOR=${IMAGE_SING_BOX_VERSION_MINOR:-0}
IMAGE_SING_BOX_VERSION_PATCH=${IMAGE_SING_BOX_VERSION_PATCH:-0}

IMAGE_TAG_FULL_QUALIFIED="${IMAGE_REPO}:${IMAGE_SING_BOX_VERSION}-r${IMAGE_BUILD_REVISION}"
IMAGE_TAG_MAJOR_MINOR_PATCH="${IMAGE_REPO}:${IMAGE_SING_BOX_VERSION_MAJOR}.${IMAGE_SING_BOX_VERSION_MINOR}.${IMAGE_SING_BOX_VERSION_PATCH}"
IMAGE_TAG_MAJOR_MINOR="${IMAGE_REPO}:${IMAGE_SING_BOX_VERSION_MAJOR}.${IMAGE_SING_BOX_VERSION_MINOR}"
IMAGE_TAG_MAJOR="${IMAGE_REPO}:${IMAGE_SING_BOX_VERSION_MAJOR}"
...
for arch in "${IMAGE_BUILD_TARGET_GOARCHS[@]}"; do
	...
	buildah build \
		...
		--build-arg IMAGE_SING_BOX_VERSION=$IMAGE_SING_BOX_VERSION \
